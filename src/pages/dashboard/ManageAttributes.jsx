...
const ManageAttributes = () => {
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
      ...
      <ul className="space-y-2 mb-6 max-h-60 overflow-y-auto no-scrollbar">
        {items?.map((item) => (
          <li
            key={item.id}
            className="flex justify-between items-center bg-gray-50 px-3 py-2 rounded-md group"
          >
        ...
        {items?.length === 0 && (
          <li className="text-xs text-gray-400 italic">No options defined.</li>
        )}
      </ul>
      <form
        onSubmit={(e) => {
      ...
      >
        ...
        <button
          type="submit"
          className="bg-amber-500 hover:bg-amber-600 text-white px-3 py-2 rounded-md transition-colors shadow-sm"
        >
  ...
  if (loading)
    return <div className="p-10 text-center">Loading attributes...</div>;
    ...
    <div className="max-w-6xl mx-auto">
      <header className="mb-8">
        ...
        <p className="text-gray-500 mt-2">
          Customize the dropdown options for Metal Type, Purity, and Color.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <AttributeSection
          title="Metal Types"
          type="metal_type"
          items={attributes.metal_type}
        />
        <AttributeSection
          title="Metal Purity"
          type="metal_purity"
          items={attributes.metal_purity}
        />
        <AttributeSection
          title="Metal Colors"
          type="metal_color"
          items={attributes.metal_color}
        />
      </div>
    </div>
  );
