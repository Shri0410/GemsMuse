    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-8 animate-slide-up">
                ...
                <Link to="/dashboard/journals/new" className="btn-luxury flex items-center gap-2">
                    <span>+ New Story</span>
                </Link>
            </div>
                            ...
                            {journals.map((journal, index) => (
                                ...
                                >
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="flex-shrink-0 h-10 w-16 rounded overflow-hidden bg-gray-100">
                                                {journal.image_url ? (
                                                    <img src={`/${journal.image_url}`} alt={journal.title} className="h-full w-full object-cover" />
                                                ) : (
                                                    <div className="h-full w-full flex items-center justify-center text-gray-300 text-xs">No Img</div>
                                                )}
                                            </div>
                                            <div className="ml-4">
                                                <div className="text-sm font-semibold text-gray-900 font-serif max-w-xs truncate">{journal.title}</div>
                                                <div className="text-xs text-gray-500 truncate max-w-xs">{journal.excerpt}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link to={`/dashboard/journals/${journal.id}`} className="text-amber-600 hover:text-amber-900 mr-4 font-semibold hover:underline">Edit</Link>
                                        <button onClick={() => handleDelete(journal.id)} className="text-gray-400 hover:text-red-600 transition-colors">Delete</button>
                                    </td>
                                </tr>
                            ))}
                            {journals.length === 0 && (
