...
const ProductDetails = () => {
    ...
    const fetchProductData = async () => {
      ...
      try {
        ...
        if (data.collection_id) {
          const relatedRes = await fetch(`/api/products?collection_id=${data.collection_id}`);
  ...
  useEffect(() => {
    const wishlist = JSON.parse(localStorage.getItem('gems_muse_wishlist') || '[]');
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-40 px-6 text-center">
        ...
        <Link to="/collection" className="bg-primary text-white px-10 py-4 uppercase text-[10px] tracking-widest font-bold">
          Back to Collection
        </Link>
      </div>
    );
          ...
          <div className="w-full lg:w-1/2">
            <div className="sticky top-40 space-y-8">
              ...
              <div className="relative aspect-square bg-[#FAF8F5] dark:bg-surface-dark flex items-center justify-center p-8 overflow-hidden shadow-sm border border-gray-50 dark:border-gray-800">
                ...
                >
                  <span className="material-icons-outlined text-lg">{isLiked ? 'favorite' : 'favorite_border'}</span>
                </button>
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 gap-4 text-sm border-t border-gray-100 dark:border-gray-800 pt-8">
                ...
                <div className="flex justify-between items-center py-2 border-b border-gray-50 dark:border-gray-800/50">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-text-muted-light font-bold">Style / SKU No</span>
                  <span className="font-medium">{product.sku || 'N/A'}</span>
                </div>

                <div className="flex justify-between items-center py-2 border-b border-gray-50 dark:border-gray-800/50">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-text-muted-light font-bold">Product Type</span>
                  <span className="font-medium">{product.product_type || 'N/A'}</span>
                </div>

                <div className="flex justify-between items-center py-2 border-b border-gray-50 dark:border-gray-800/50">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-text-muted-light font-bold">Set Info</span>
                  <span className="font-medium italic">{product.set_name || 'Standalone Piece'}</span>
                </div>

                <div className="grid grid-cols-2 gap-x-12 mt-4">
                  <div className="space-y-4">
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Metal</span>
                      <span className="text-sm">{product.metal_type || 'Gold'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Metal Purity</span>
                      <span className="text-sm">{product.metal_purity || '18K'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Metal Color</span>
                      <span className="text-sm">{product.metal_color || 'Yellow'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Metal Weight</span>
                      <span className="text-sm">{product.metal_weight ? `${product.metal_weight}g` : 'N/A'}</span>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Gem Stones</span>
                      <span className="text-sm">{product.gem_stones || 'N/A'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Centre Stone</span>
                      <span className="text-sm">{product.center_stone_weight ? `${product.center_stone_weight} cts` : 'N/A'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Total Stone Wt.</span>
                      <span className="text-sm">{product.total_stone_weight ? `${product.total_stone_weight} cts` : 'N/A'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-1">Diamond Weight</span>
                      <span className="text-sm">{product.total_diamond_weight ? `${product.total_diamond_weight} cts` : 'N/A'}</span>
                    </div>
                  </div>
                </div>

                <div className="mt-6 flex flex-col py-4 bg-gray-50 dark:bg-black/20 px-4 border-l-2 border-primary">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-text-muted-light font-bold mb-2">Available Size</span>
                  <span className="text-sm font-medium">{product.size || 'Custom Fit Available'}</span>
                </div>

                <div className="mt-8 space-y-3">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-text-muted-light font-bold">Description</span>
              ...
              <div className="pt-10 flex flex-col gap-4">
                <div className="flex gap-4">
                  <div className="flex items-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-black/20">
                    <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-5 py-3 font-bold hover:text-primary transition-colors">-</button>
                    <span className="px-4 text-sm font-bold w-12 text-center">{quantity}</span>
                    <button onClick={() => setQuantity(q => q + 1)} className="px-5 py-3 font-bold hover:text-primary transition-colors">+</button>
                  </div>
                  <button
                    onClick={addToBag}
                ...
                <Link to="/bespoke" className="w-full text-center py-4 border border-gray-200 dark:border-gray-800 text-[10px] uppercase tracking-widest font-bold hover:bg-gray-50 dark:hover:bg-black/20 transition-all">
      ...
      <section className="py-24 bg-[#FAF8F5] dark:bg-[#151515] border-t border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-primary text-[10px] font-bold tracking-[0.5em] uppercase block mb-4">You May Also Desire</span>
              ...
              <Link to={`/product/${p.id}`} key={p.id} className="group text-center">
                <div className="relative overflow-hidden bg-white dark:bg-surface-dark mb-6 aspect-square flex items-center justify-center p-8 transition-all duration-500 hover:shadow-xl border border-gray-50 dark:border-gray-800/50">
                  <img
                    ...
                    src={p.main_image ? `/${p.main_image}` : (p.image || "https://via.placeholder.com/300x300?text=No+Image")}
                  />
                </div>
